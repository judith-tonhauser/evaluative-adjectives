...
Two contents of evaluative adjective sentences, like {\em Kim was smart to watch the movie}, are the prejacent, that Kim watched the movie, and the generalization, that the degree to which Kim watching the movie is smart is higher than the contextual standard of {\em smart}. The prejacent is standardly analyzed as a presupposition: {\em Kim wasn't smart to
watch the movie} is taken to presuppose that Kim watched the movie
(e.g., \citealt{norrick78,barker02,oshima09b,kertz2010}). This paper argues against such analyses because, among other things, they do not capture an interaction between the prejacent and the generalization that has not yet been observed for projective content: when the prejacent projects, the generalization does not, and when the prejacent does not, the generalization does! We develop an analysis according to which the prejacent is not a lexically specified presupposition but projects if it is not at-issue with respect to the question addressed by the utterance of the evaluative adjective sentence. In addition to capturing the peculiar interaction between the prejacent and the generalization, our question-based projection analysis significantly expands on previous such analyses (e.g., \citealt{beaver-clark08,best-question,brst-ar,abrusan2011}) by incorporating a novel constraint on the question addressed by an utterance: the more an utterance content follows from the common ground, the less likely the question is about that content. We provide experimental evidence for the proposed analysis and argue that our analysis improves on that of \citealt{karttunen-etal2014}, according to which evaluative adjective sentences are systematically ambiguous. 
...
dance on the table}); the subject of the predicate {\em be stupid} is the understood subject of the {\em to-}infinitive (e.g., \citealt{wilkinson70, norrick78,barker02,kertz2010}). One of the contents standardly discussed in the literature is (what we refer to as) the prejacent: in (\ref{f}), the prejacent is that Feynman danced on the table.\footnote{This paper ignores the subject-related presuppositions discussed in \citealt{barker02}.} 

\begin{exe} 
\ex\label{f} Feynman was stupid to dance on the table. \hfill (\citealt[18]{barker02})
\end{exe} 
The prejacent has traditionally been analyzed as presupposition (e.g., \citealt{norrick78,barker02,oshima09b,kertz2010}). On formal analyses in this tradition, evaluative adjectives like {\em stupid} lexically specify that the prejacent
...
The second problem concerns content other than the prejacent: here, the two previous analyses that explicitly consider content other than the prejacent, \citealt{oshima09b} and \citealt{barker02}, do not make sufficiently strong predictions. Consider first \citealt{oshima09b}. On this analysis, the prejacent is a presupposition and what is asserted is the following complex content: from the prejacent it can be inferred that the denotation of the subject is in the extension of the evaluative adjective (p.371). That is, (\ref{f2}a) is predicted to presuppose that Feynman danced on the table and to assert that it cannot be inferred from Feynman dancing on the table that Feynman is stupid. Similarly then, (\ref{nat}b) is predicted to convey that the speaker did not stumble through the junkyard (if the prejacent is locally accommodated) and to assert that it cannot be inferred from the speaker stumbling through the junkyard that the speaker is stupid. This assertion is too weak: (\ref{nat}b) means that the speaker stumbling through the junkyard would be stupid. Next consider \citealt{barker02}: on this dynamic semantic analysis, the update effect of a NEAS like (\ref{f2}a) is to filter out those worlds in which the contextual standard of {\em stupid} is too high for Feynman's participation in the event of dancing on the table to count as stupid (p.25); only those worlds remain in which Feynman dancing on the table is not considered stupid. Similarly then, the update effect of (\ref{nat}b), in which the prejacent is locally accommodated, is to filter out those worlds in which the contextual standard of {\em stupid} is too high for the speaker's participation in the event of stumbling through the junkyard to count as stupid. The worlds that remain are ones in which the speaker does not stumble through the junkyard and in which the speaker stumbling through the junkyard is not stupid. However, as mentioned above, this is not what (\ref{nat}b) is understood to mean. 
...
\citepos{karttunen-etal2014} analysis improves on previous ones because it is able to derive both attested interpretations. It does so by assuming that evaluative adjectives are ambiguous: the prejacent is specified as presupposed in the lexical entry in  (\ref{lex}a) but not in (\ref{lex}b). 
...
%{\bf Adj1: prejacent presupposed, evaluation asserted, so it doesn't project}
%
%{\bf Adj2: evaluation presupposed, asserted content is under negation, so prejacent is denied, doesn't project}



The lexical entry in (\ref{lex}a) correctly predicts that the NEAS in (\ref{f2}a) has an interpretation according to which the prejacent, that  Feynman danced on the table, projects. Furthermore, what is asserted is that it is not the case that for Feynman to dance on the table would be stupid, from which it follows, together with the prejacent,  that it is not the case that Feynman dancing on the table was stupid (\citealt[248]{karttunen-etal2014}). In contrast, the lexical entry in (\ref{lex}b) predicts that the NEAS in (\ref{nat}b) has an interpretation according to which it is presupposed that for the speaker to stumble through the junkyard would be stupid and for the speaker to not stumble through the junkyard would not be stupid; thus, in contrast to \citepos{oshima09b} and \citepos{barker02} analyses, this analysis correctly predicts that (\ref{nat}b) conveys that the speaker stumbling through the junkyard would be stupid. What is asserted is that it is not the case that what the speaker did about stumbling through the junkyard was stupid. That the prejacent is negated (the speaker didn't stumble through the junkyard) follows, according to \citealt[249]{karttunen-etal2014}, from what is presupposed and what is asserted.

\citepos{karttunen-etal2014} analysis improves on previous ones because it is able to derive both attested interpretations. There are, however, two conceptual worries with the analysis. First, the analysis comes at a high cost: to derive the two interpretations, evaluative adjectives are systematically ambiguous. A second conceptual worry is that the analysis hard-wires into the two lexical entries an interaction between the projectivity of the prejacent and the projectivity of (what we refer to as) the generalization that would preferably fall out from a projection analysis. To illustrate, consider again the examples in (\ref{f2}a) and (\ref{nat}b). The generalization of (\ref{f2}a) is that the degree to which Feynman dancing on the table is stupid is higher than the contextual standard of {\em stupid}; that of (\ref{nat}b) is that the degree to which the speaker stumbling through the junkyard in the dark is stupid is higher than the contextual standard of {\em stupid}. (We motivate in section \ref{s2} that both the prejacent and the generalization are lexical entailments of EAS.) There is an interaction between the  prejacent and the generalization that has not yet been observed for projective content: when the prejacent projects, the generalization does not, as in (\ref{f2}b), and when the prejacent does not project, the generalization does, as in (\ref{nat}b). To illustrate that this interaction is specific to the projective content associated with EAS, consider (\ref{noint}a) with the factive predicate {\em discover}: what is typically taken to follow from (\ref{noint}a) is that the T.A.\ was cheating and that Sam discovered that the T.A.\ was cheating. As is well-known, the content of the complement of {\em discover} may project over negation, in which case (\ref{noint}b) means that the T.A.\ was cheating but that Sam didn't discover it. Crucially, when the content of the complement does not project over negation, that Sam discovered it does not project either: (\ref{noint}b) means that it is not the case that the T.A.\ was cheating and Sam discovered it, i.e., both contents of (\ref{noint}a) are interpreted in the scope of negation. 

\begin{exe}
\ex\label{noint}
\begin{xlist}
\ex Sam discovered that the T.A.\ was cheating.
\ex Sam didn't discover that the T.A.\ was cheating.
\end{xlist}
\end{exe}
With EAS, by contrast, we observe an interaction between the projection of the prejacent and the generalization. This interaction is hard-wired into the lexical entries under \citepos{karttunen-etal2014} analysis: when the prejacent projects, as in the lexical entry {\em adj}$_1$ in (\ref{lex}a), the generalization does not, by virtue of following from the asserted content; and when the prejacent does not project, as in the lexical entry {\em adj}$_2$ in (\ref{lex}b), the generalization does, by virtue of being part of the presupposed content. 



%A third conceptual worry with \citepos{karttunen-etal2014} analysis concerns a content we refer to as the evaluation: in NEASs, the evaluation is negated when the prejacent projects but counterfactual when the prejacent does not project: that is, the evaluation of (\ref{f2}a), where the prejacent projects, is that Feynman dancing on the table was stupid, whereas the evaluation of (\ref{nat}b), where the prejacent does not project, is that the speaker stumbling through the junkyard would be stupid. This behavior of the evaluation, too, is hard-wired into the lexical entries of \citepos{karttunen-etal2014} analysis: the negated evaluation follows from the presupposed prejacent and the negated assertion in the lexical entry {\em adj}$_1$ in (\ref{lex}a), whereas the counterfactual evaluation is directly coded as part of the presupposition in the lexical entry {\em adj}$_2$ in (\ref{lex}b).

The analysis of EAS we develop in section \ref{s2} correctly predicts the two attested interpretations of EASs, in contrast to analyses according to which the prejacent is lexically specified as a presupposition, and also avoids both of the conceptual worries with \citepos{karttunen-etal2014} analysis: there is a single lexical entry for each evaluative adjective and the observed interaction in the projection of the prejacent and the generalization falls out from the analysis. After developing the analysis in section \ref{s2}, we provide experimental evidence for two predictions in section \ref{s3} and provide tentative support for a third prediction in section \ref{s4}. {\bf We discuss two open issues in section \ref{s5} NEED TO ADJUST} and conclude the paper in section \ref{s6}.

\section{A question-based analysis of the projection of the prejacent}\label{s2}

In this section, we develop an analysis that correctly predicts that the prejacent of EAS can, but need not project, and that also captures the interaction between the projection of the prejacent and the generalization. In contrast to previous analyses, including that in \citealt{karttunen-etal2014}, our analysis does not assume that the prejacent projects because it is lexically specified as a presupposition. 

There are currently several analyses on the market that derive the projection of utterance content. 


Some, discussed above, derive projection from a lexical specification of the content as a presupposition (e.g., \citealt{heim83,vds92}). Others derive projection from a combination of conventionally specified alternatives and pragmatic mechanisms (e.g., \citealt{abusch02,abusch10,romoli2015}) 

 from default backgrounding (e.g.\ \citealt{abrusan2011,abrusan2016}) or from the 

More recently, alternative analyses have been proposed. On Abusch's (2002, 2010)  and Romoli's (2011, 2015) analyses, for instance, the projectivity of the pre-state content of {\em stop} is derived from conventionally specified alternatives to {\em stop} (these analyses differ in their details, as discussed in section \ref{s4}.)  \citet{abrusan2011,abrusan2016}, in turn, proposed that the pre-state content projects because it is by default backgrounded, and \citet*{brst-ar} suggested that the pre-state content projects if and only if it is not at-issue with respect to the QUD (see also \citealt*{brst-salt10,tbd-variability}). 
...
In this section, we develop an analysis of the projection of the prejacent of EAS that does not rely on specifying in the lexical entry of the evaluative adjective that the prejacent is a presupposition. 
...
According to our analysis of the projection of the prejacent of EASs, each evaluative adjective has only one lexical entry and this lexical entry does not specify that the prejacent is a presupposition. Because the prejacent is not lexically specified as presupposed, we expect the prejacent to not be highly projective. 

Critically, the lexical entry of an evaluative adjective does not specify that the prejacent is a presupposition. Empirical support for this assumption comes from the observation that the prejacent is not highly projective. First, AUTHORS' 2018 experimental investigation observed that the projectivity of the prejacent of EASs with {\em stupid} was significantly lower than that of the content of the complement of the factive predicates {\em be annoyed} and {\em know}. Second, in naturally occurring data, too, the prejacent of NEASs is not highly projective. We conducted a corpus-based web study that collected `certainty' ratings from native speakers of American English for the prejacents of 59 naturally occurring NEAS with seven evaluative adjectives: the higher the certainty rating, the more projective the prejacent. Figure \ref{f-corpus} shows the mean certainty ratings of the 59 NEAS by evaluative adjective: across the 59 NEASs, the prejacent is not highly projective; the mean certainty rating was only 3.2. (This corpus study is described in detail in Appendix \ref{a-corpus}.)
...
In this section, we develop an analysis according to which the prejacent of an EAS projects when the prejacent is not at-issue with respect to the question addressed by the utterance of the EAS (see, e.g., \citealt{brst-salt10,brst-ar,best-question}). 


Question-based analyses of the projectivity of utterance content have been developed for the content of the complement of factive predicates in \citealt{best-question}; see also \citealt{brst-salt10} and \citealt{brst-ar} for discussion.

To understand which questions EASs can address, we first consider the lexical entailments of EASs.


\subsection{Lexical entailments of evaluative adjective sentences}\label{s21}
...
\subsection{At-issue content of utterances of evaluative adjective sentences}\label{s22}
...
In the next section, we provide experimental support for this analysis. 

\section{The discourse status of the generalization}\label{s3}
